...
static int next_stats(struct ifile_info *iip, struct blkio_stats *stats)
{
	size_t rd = 0, rm = sizeof(*stats);

	memset(stats, 0, sizeof(*stats));
	while (rm) {
		ssize_t ret = read(iip->ifd, (char *)stats + rd, rm);
		if (ret <= 0)
			return 0;
	...
	if (iip->genesis == 0) {
		uint64_t gen = 0;
		if (stats->read.total_ops)
			gen = stats->read.start;
		if (stats->write.total_ops && stats->write.start < gen)
			gen = stats->write.start;
		iip->genesis = gen;
	}
	return 1;
}
...
static uint64_t poor_random(uint64_t low, uint64_t high)
{
	const uint64_t rnd = ((uint64_t)rand() << 32) | (uint64_t)rand();
	assert(low <= high && "High is less than low");
	return (rnd % (high - low + 1)) + low;
}

static void generate_ios(struct io_stream *stream, struct blkio_stats *stats)
{
	const struct blkio_stat *rd = &stats->read;
	const struct blkio_stat *wr = &stats->write;
	size_t reads = rd->total_ops;
	size_t writes = wr->total_ops;
	uint64_t rd_time = rd->start;
	uint64_t wr_time = wr->start;
	...
	if (reads) {
		rd_time_d = (rd->end - rd_time) / reads;
		rd_bytes = rd->total_bytes / reads;
	}

	if (writes) {
		wr_time_d = (wr->end - wr_time) / writes;
		wr_bytes = wr->total_bytes / writes;
	}

	while (reads || writes) {
		if (reads && (!writes || rd_time < wr_time)) {
			spec.time = rd_time;
			spec.sector = poor_random(rd->min_sector,
				rd->max_sector);
			spec.bytes = rd_bytes;
			spec.rw = 0;
			...
			spec.time = wr_time;
			spec.sector = poor_random(wr->min_sector,
				wr->max_sector);
			spec.bytes = wr_bytes;
			spec.rw = 1;
...
static void process(struct ifile_info *iip)
{
	...
	while (next_stats(iip, &stats))
		generate_ios(stream, &stats);
	stream_close(stream);
